...
ggplot(d, aes(Answer.time_in_minutes)) +
   geom_histogram()
ggsave(file="../graphs/2_norm/answer_time_in_minutes.png")

#gender breakdown of trials (not by participant)
ggplot(d, aes(gender)) +
   stat_count()
ggsave(file="../graphs/2_norm/participant_gender.png")

#did they understand the HIT?
ggplot(d, aes(asses)) +
   stat_count()
ggsave(file="../graphs/2_norm/participant_understood_task.png")

# age by trial
ggplot(d, aes(age)) +
   geom_histogram()
ggsave(file="../graphs/2_norm/age_by_trial.png")
...
ggplot(d, aes(education)) +
   stat_count()
ggsave(file="../graphs/2_norm/education_by_trial.png")
...
#people that failed the attention check: id 35
#people whose native language isn't English: id 14
norm = d %>%
  filter(block == "context_norming") %>%
  filter(workerid != '35' & workerid != '14')%>%
  droplevels()

#plotting a bar plot of polar versus exhaustive responses, for polar versus exhaustive QUD
ggplot(norm, aes(response)) + 
  geom_bar(stat="count") + 
  facet_wrap(~qud) +
  theme(axis.text.x=element_text(angle=45,hjust=1,vjust=1))

#plotting exhaustive versus polar responses, per scenario
ggplot(norm, aes(x=qud,fill=response)) +
  geom_bar(stat="count",position='dodge') +
  facet_wrap(~scenario) +
  theme(axis.text.x=element_text(angle=45,hjust=1,vjust=1))

#plotting exhaustive versus polar responses, per participant
ggplot(norm, aes(x=qud,fill=response)) +
  geom_bar(stat="count",position='dodge') +
  facet_wrap(~workerid) +
  theme(axis.text.x=element_text(angle=45,hjust=1,vjust=1))

#Make a plot of prior beliefs using last norming experiment
#Proportion of polar versus exhaustive for polar context against prior probability

#Obtain an index of susceptibility to QUD manipulation: per individual, proportion of exhaustive responses in polar context / proportion of exhaustive responses in exhaustive context

#For half items, interpretation (random assignment of QUD so that half half), for half, replicate QUD experiment
#interpretation block, QUD block

#Model predicts exhaustivity rating from QUD * QUD-susceptibility-index * priors (can be used from first norming experiment) + block_order

#for CAMP: experimental stuff

#pre-register with OSF

#think about how to formulate a QUD susceptibility index
# - write down the different ways it could be computed, some justification for a choice
